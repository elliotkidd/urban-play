...
function Team({ title, teamMembers, smallWrapper }: TeamProps) {
  return (
    ...
    >
      ...
      >
        {teamMembers.map(({ _id, image, name, position, startingYear }, i) => (
          <motion.li
            key={`team-member-${_id}-${i}`}
            variants={childVars}
            className={twMerge(
              "font-bold",
              i % 10 === 3 ? "lg:col-start-1" : "",
              i % 10 === 7 ? "lg:col-start-2" : "",
            )}
          >
            <div className="relative aspect-portrait overflow-hidden mb-4">
              ...
              />
              {startingYear && (
                <span className="absolute bg-nav-bar-background/20 backdrop-blur text-white top-3 left-3 text-xs font-bold p-4 rounded-xl">
                  Since {startingYear}
                </span>
              )}
            </div>
