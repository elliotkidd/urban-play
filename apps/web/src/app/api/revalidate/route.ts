...
import { revalidateReferences } from "@/sanity/loader/loadQuery";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body?._id) {
      const relations = await revalidateReferences(body._id);

      if (relations) {
        relations.data.map((relation) => {
          revalidateTag(relation._type);
          console.log("Revalidating related " + relation._type);
          if (relation.slug) {
            revalidateTag(`${relation._type}:${relation.slug}`);
            console.log(
              "Revalidating related " + relation._type + ": " + relation.slug,
            );
          }
        });
      }
